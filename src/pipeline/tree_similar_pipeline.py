...
class TreeSimilarPipeline(PipelineTemplate, HousePriceFeatureManager):
    ...
    def _preprocess_features(self, df) -> pd.DataFrame:
        df = self.__features_to_rates(df)
        df = self.__features_to_others(df)
        df = self.__feautures_to_is(df)
        
        return df
    
    def __features_to_rates(self, df) -> pd.DataFrame:
    ...
    def __features_to_others(self, df) -> pd.DataFrame: 
    ...
    def __feautures_to_is(self, df) -> pd.DataFrame:
